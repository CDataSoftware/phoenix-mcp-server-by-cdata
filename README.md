...
:heavy_exclamation_mark: This project builds a read-only MCP server. For full read, write, update, delete, and action capabilities and a simplified setup, check out our free [CData MCP Server for Phoenix (beta)](https://www.cdata.com/download/download.aspx?sku=UAZK-V&type=beta). 
## Purpose
...
5. Create a `.prp` file for your JDBC connection (e.g. `phoenix.prp`) using the following properties and format:
